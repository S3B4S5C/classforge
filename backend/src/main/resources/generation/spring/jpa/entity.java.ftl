...
</#if>public class ${entity.className}<#if entity.inheritance.kind?string == "JOINED_SUBCLASS"> extends ${entity.inheritance.superClassName}</#if> {
...
</#list><#if entity.inheritance.kind?string == "JOINED_SUBCLASS"><#if entity.inheritance.primaryKeyJoinColumns?size == 1>    @PrimaryKeyJoinColumn(name = "${entity.inheritance.primaryKeyJoinColumns[0].localColumnName}", referencedColumnName = "${entity.inheritance.primaryKeyJoinColumns[0].referencedColumnName}")
...
</#if></#if><#list entity.directRelations as relation><#if relation.onDeleteCascade>    @OnDelete(action = OnDeleteAction.CASCADE)
